...
function selectCheckBox() {
    ...
    document.getElementById("textKey").value = w_textKey;          //A
    document.getElementById("textMemo").value = w_textMemo;         // B
    if (w_cnt === 1) {                                              // C
        return (w_sel = "1");                                         // C
    } else {
        window.alert("１つ選択（select）してください。");              // C
    }
}
...
function viewStorage() {
    ...
    for (let i = 0; i < localStorage.length; i++) {// localStorage すべての情報の取得
        ...
        //td1 にラジオボタン、td2 にキー
        td1.innerHTML = '<input name="chkbox1" type="checkbox">';//D
        td2.innerHTML = w_key;
